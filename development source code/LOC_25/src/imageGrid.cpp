...
//    portrait_gridFont.loadFont("HELVETICALTSTD-BOLDCOND.OTF", portrait_fontSize, false);
    ...
    portrait_gridFontSmall.setup("HELVETICALTSTD-COND.ttf"); //, portrait_fontSizeSmall);
     portrait_gridFontSmall.addFont("HELVETICALTSTD-COND.ttf");
...
void imageGrid::drawOnePortraitSmall(int i, ofPoint pos,float scaler){
    ...
    ofDrawRectangle(portrait_imagesSmall[i].getWidth()*scaler, portrait_imagesSmall[i].getHeight()*scaler, 3, -temp_predictionAmt);
...
void imageGrid::loadStudentNames(){
    ...
    ofBuffer buffer = ofBufferFromFile(ofToDataPath("names.txt"));
