...
bool ofxFaceTracker2::update(Mat image, cv::Rect _roi) {
    ...
    if(im.type() == CV_8UC1) {
        ...
        cvtColor(im, gray, COLOR_RGB2GRAY); //CV_RGB2GRAY);
    }
	imageDirty = true;
...
void ofxFaceTracker2::runLandmarkDetector(){
    ...
    if(faceRectanglesTracker.getCurrentLabels().size() != 0){
        ...
        for(auto label : faceRectanglesTracker.getCurrentLabels()){
            ...
            auto lastSeen = faceRectanglesTracker.getLastSeen(label);

            
            // Do the actual landmark detection
            dlib::full_object_detection shape = landmarkDetector(dlibimg, rect);

            instances.push_back(ofxFaceTracker2Instance(label,age, lastSeen, shape, rect, info));
        }
...
void ofxFaceTracker2::drawDebug(int x, int y, bool _justBox) const{
    drawDebug(x, y, info.inputWidth, info.inputHeight, _justBox);
}

void ofxFaceTracker2::drawDebug(int x, int y, int _w, int _h, bool _justBox) const{
    if(failed) {
        return;
    }
...
void ofxFaceTracker2::drawAvg(int x, int y, int _w, int _h, float _avg){
    //    ofLog()<<"_avg "<<_avg;
    if(failed) {
        return;
    }
...
//            vector<ofPoint> tempArray = instance.getLandmarks().getImageFeature(ofxFaceTracker2Landmarks::LEFT_EYE).getVertices();
            auto tempArray = instance.getLandmarks().getImageFeature(ofxFaceTracker2Landmarks::LEFT_EYE).getVertices();

            ofPolyline tempPL;
            if(last_LEFT_EYE.size() != tempArray.size()){
                last_LEFT_EYE.resize(tempArray.size());
            }
            
            for(int j=0; j< tempArray.size(); j++){
                
                ofLog()<<"(tempArray[j] - last_LEFT_EYE[j]) * _avg "<<((tempArray[j] - last_LEFT_EYE[j]) * _avg);
                last_LEFT_EYE[j] += (tempArray[j] - last_LEFT_EYE[j]) * _avg;
                
                ofLog()<<"last_LEFT_EYE[j] "<<last_LEFT_EYE[j]<<" tempArray[j] "<<tempArray[j]<<" ,  _avg "<<_avg;
                tempPL.addVertex(last_LEFT_EYE[j]);
            }
            tempPL.draw();
            
            
            tempArray = instance.getLandmarks().getImageFeature(ofxFaceTracker2Landmarks::RIGHT_EYE).getVertices();
            tempPL.clear();
            if(last_RIGHT_EYE.size() != tempArray.size()){
                last_RIGHT_EYE.resize(tempArray.size());
            }
            for(int j=0; j<tempArray.size(); j++){
                last_RIGHT_EYE[j] += (tempArray[j] - last_RIGHT_EYE[j]) * _avg;
                tempPL.addVertex(last_RIGHT_EYE[j]);
            }
            tempPL.draw();
            ...
            tempArray = instance.getLandmarks().getImageFeature(ofxFaceTracker2Landmarks::JAW).getVertices();
            tempPL.clear();
            if(last_JAW.size() != tempArray.size()){
                last_JAW.resize(tempArray.size());
            }
            for(int j=0; j<tempArray.size(); j++){
                last_JAW[j] += (tempArray[j] - last_JAW[j]) * _avg;
                tempPL.addVertex(last_JAW[j]);
            }
            tempPL.draw();
            cnt++;
        }
...
void ofxFaceTracker2::drawByLabel(int _label, int x, int y, int _w, int _h) const{
...
ofPolyline ofxFaceTracker2::getLeftEye(int _label) const{
    for (auto instance : getInstances()){
        if(instance.getLabel() == _label){
            return instance.getLandmarks().getImageFeature(ofxFaceTracker2Landmarks::LEFT_EYE);
            break;
        }
    }
    ofPolyline nothing;
...
ofPolyline ofxFaceTracker2::getRightEye(int _label) const{
    for (auto instance : getInstances()){
        if(instance.getLabel() == _label){
            return instance.getLandmarks().getImageFeature(ofxFaceTracker2Landmarks::RIGHT_EYE);
            break;
        }
    }
...
ofPolyline ofxFaceTracker2::getNoseBridge(int _label) const{
    for (auto instance : getInstances()){
        if(instance.getLabel() == _label){
            return instance.getLandmarks().getImageFeature(ofxFaceTracker2Landmarks::NOSE_BRIDGE);
            break;
        }
    }
    ofPolyline nothing;
...
ofPolyline ofxFaceTracker2::getImageFeature(int _label, ofxFaceTracker2Landmarks::Feature _name) const{
    for (auto instance : getInstances()){
        if(instance.getLabel() == _label){
            return instance.getLandmarks().getImageFeature(_name);
            break;
        }
    }
    ofPolyline nothing;
...
ofRectangle ofxFaceTracker2::getBoundingBox(int _label) const{
    for (auto instance : getInstances()){
        if(instance.getLabel() == _label){
...
int ofxFaceTracker2::getOldestLabel() const{
    
    int oldest = INT_MAX;
    for (auto instance : getInstances()){
        if(instance.getLabel() < oldest) oldest = instance.getLabel();
    }
    return oldest;
}

int ofxFaceTracker2::getOldestLabel_withOrientationLimit(int _maxOrientation) const{
...
void ofxFaceTracker2::setMaximumDistance(int _dist){
    faceRectanglesTracker.setMaximumDistance(_dist);
}
void ofxFaceTracker2::setPersistence(int _frames){
    faceRectanglesTracker.setPersistence(_frames);
}
