...
namespace ofxProsilica {
	...
	class ConnectorWithInterfaceThreaded : public Connector, public ofThread {
	public:
		
        ConnectorWithInterfaceThreaded(){
            
        }
        
        ~ConnectorWithInterfaceThreaded(){
            if(isThreadRunning()) {
                ofLog(OF_LOG_ERROR, "~ConnectorWithInterfaceThreaded :: Thread was not stopped. You must call waitForThread() in ofApp::exit() or exit() of class that holds this object.");
            }
        }
        bool setup(){
            startThread(true);
